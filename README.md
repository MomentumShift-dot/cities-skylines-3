...
- **수요(RCI-O)** — **밀도별로 따로 계산합니다.** 주거는 저/중/고, 상업은 저/고가 각각
  자기 공실률과 선호도를 가집니다. 도시가 작고 땅값이 싸면 단독주택 수요가,
  도시화·토지가치가 오르면 중·고밀도 수요가 커집니다.
  실업률 피드백도 들어 있어 일자리가 모자라면 상업·산업 수요가, 집이 모자라면 주거 수요가 오릅니다.
  화면 우측 상단 막대가 `주거(저·중·고) / 상업(저·고) / 산업 / 사무` 7종으로 나뉘어 표시되고,
  잠긴 밀도는 흐리게 표시됩니다.

### 🖥️ HUD
상단바는 `자금 · 인구 · 만족도 · 실업률` 4개 지표와, **인프라 미니 게이지**
(⚡ 전력 · 💧 상수도 · 🚽 하수 · 🗑️ 쓰레기 · 🚗 교통)로 구성됩니다.
게이지는 충족률을 색으로 보여주고(초록/노랑/빨강), 부족하면 아이콘이 깜빡이며,
마우스를 올리면 정확한 수치가 나옵니다.
...
# 브라우저 전 기능 점검 (42개 항목 — 건물 모델 147종 렌더링·툴바 팔레트·낮밤 주기 검증 포함)
